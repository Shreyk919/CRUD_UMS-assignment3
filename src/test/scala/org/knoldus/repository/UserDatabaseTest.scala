...
class UserDatabaseTest extends AnyFlatSpec {
   ...
   * if USERID is given
   */
  val User2: User = User(Some(UUID.randomUUID()), "Abhay", userType.Customer)
  ...
  */
  ...
  "updateUser method" should "return TRUE if it successfully updates the User details" in {
    ...
    val updatedUser= userDB.readUserbyID(User1ID).copy(name = "Vedant")
  ...
  it should "throw a NoSuchElementException if the given USERID does not exist" in {
    ...
    val updatedUser: User = userDB.readUserbyID(User1ID).copy(name = "Vedant")
  ...
  "readAll method" should "return all the users stored in a list" in {

    val outputList = userDB.readAll
    assert(outputList == List(userDB.readUserbyID(User1ID)))
  }
  /**
   * and
   */
